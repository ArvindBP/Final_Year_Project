<?php
	...
	if (!empty($_GET['latitude']) && !empty($_GET['longitude']) &&
		...
		!empty($_GET['speedOTG']) && !empty($_GET['course']) && !empty($_GET['Name']) && !empty($_GET['Number'])) {
		...
		$sql = "INSERT INTO gpscoord (number,name,latitude, longitude,timer,speed,sat,course,servicable)
		VALUES ('$number','$name','$lat', '$lon','$time','$speed','$sat','$course','Yes')";
...
<head>
	...
	<script src="http://maps.googleapis.com/maps/api/js?key=AIzaSyDRATAdxhXsZt_0W0h1U4LwmexESa0XpKk&sensor=false"></script>
